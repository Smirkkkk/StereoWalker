...
def _read_walk_ids(walk_list_path):
    with open(walk_list_path, "r") as f:
        return set(ln.strip() for ln in f if ln.strip())

def _key_from_filename(fn):
    # fn: ".../_9NBtbTq1kE_15081748.npz" -> key: "_9NBtbTq1kE"
    stem = os.path.splitext(os.path.basename(fn))[0]
    return stem.rsplit("_", 1)[0]  

def _collect_npz(roots, id_set):
...
def get_c2w(extr):
    ...
    """
    w2c = np.concatenate((extr, np.array([[0, 0, 0, 1]])), axis=0)
...
class Stereo4DDataset(Dataset):
    def __init__(self, cfg, mode):
        ...
        if mode == "train":
            self.video_dir = os.path.join(cfg.data.video_dir, "train_mp4s")
            roots = [os.path.join(self.pose_dir, "train")]
            walk_list = cfg.data.train_list
        elif mode == "val" or mode == "test":
            self.video_dir = os.path.join(cfg.data.video_dir, "test_mp4s")
            roots = [os.path.join(self.pose_dir, "test")]
            walk_list = cfg.data.test_list
        else:
            raise ValueError(f"Invalid mode {mode}")
        id_set = _read_walk_ids(walk_list)
        self.pose_path = _collect_npz(roots, id_set)
        ...
        for f in self.pose_path:
            ...
            video_file = f"{key}-left_rectified.mp4"
            video = os.path.join(self.video_dir, video_file)
            if not os.path.exists(video):
                raise FileNotFoundError(f"Video file {video} does not exist.")
            self.video_path.append(video)
        ...
        for f in tqdm(self.pose_path, desc="Loading poses"):
            ...
            dp = load_dataset_npz(f)
            extrs_rectified = dp['extrs_rectified']
            # pose = [get_c2w(extr) for extr in extrs_rectified]
            bottom = np.broadcast_to(np.array([0,0,0,1], dtype=extrs_rectified.dtype), (extrs_rectified.shape[0], 1, 4)) # (N,1,4)
            pose = np.linalg.inv(np.concatenate([extrs_rectified, bottom], axis=1))  # (N,4,4)
        ...
        for pose in self.poses:
            step_scale = np.linalg.norm(np.diff(pose[:, [0, 2], -1], axis=0), axis=1).mean()
    ...
    def __getitem__(self, index):
        ...
        if self.cfg.model.cord_embedding.type == 'polar':
            ...
            transformed_input_positions = np.concatenate([
                ...
                self.transform_pose(target_pose, current_pose)[np.newaxis, [0, 2]]
            ], axis=0)
        else:
            raise NotImplementedError(f"Coordinate embedding type {self.cfg.model.cord_embedding} not implemented")
        waypoints_transformed = self.transform_poses(waypoint_poses, current_pose)
    ...
    def old_transform_poses(self, poses, current_pose_array):
    ...
    def transform_poses(self, poses, current_pose_array):
        current_pose_inv = np.linalg.inv(current_pose_array)         # (4,4)
        transformed_matrices = np.matmul(current_pose_inv[np.newaxis, :, :], poses)  # (N,4,4)
        positions = transformed_matrices[:, :3, 3]
        return positions
    
    def transform_pose(self, pose, current_pose_array):
        current_pose_inv = np.linalg.inv(current_pose_array)         # (4,4)
        transformed_matrix = np.matmul(current_pose_inv, pose)  # (4,4)
